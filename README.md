...
Дано:
•	Разрабатываемая система обрабатывает сущности User, Story, Label и Comment, которые логически связаны отношениями
o	один-ко-многим (User и Story, Story и Comment)
o	многие-ко-многим (Story, Label).
•	Необходимо выполнить перенос хранения сущности Comment из реляционной базе данных Postgres в новый модуль/микросервис с другой базой данных, а именно Cassandra или MongoDB
Задание
В ранее разработанном приложении необходимо выделить обсуждения на основе сущности Comment в отдельный модуль discussion. Для корректного распределения информации в нодах базы данных следует изменить структуру сущности Comment согласно схеме. Вместо модуля допускается разработка отдельного микросервиса discussion.
...
o	Префикс драйвера jdbc:cassandra:
o	Адрес базы данных localhost
o	Порт базы данных 9042
o	user не нужен
o	password не нужен
o	Схема distcomp
...
1.	Если это еще не сделано преобразуйте проект в многомодульный. Структура произвольная, но минимально должно быть два модуля: publisher и discussion
...
o	Создать контроллеры для обработки HTTP запросов через вспомогательный REST-адрес localhost:24130
o	Настроить маршрутизацию запросов к соответствующим методам контроллеров
